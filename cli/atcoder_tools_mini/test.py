...
def get_test_commands(src_path):
    ...
    if not symbol_found:
        print(f"\033[91m[CLI] Error: File extension '{ext}' is not supported for local testing.\033[0m")
        sys.exit(1)
    ...
    if os.path.isfile(config_path):
        try:
            ...
            print(f"\033[93m[CLI] Warning: Failed to parses {config_path} -> {e}\033[0m")
...
def run_tests(args):
    ...
    if compile_template:
        ...
        try:
            ...
            print("\033[91m[CLI] Compilation Failed!\033[0m")
            print(e.stderr)
            return False
    ...
    if not os.path.isdir(in_dir) or not os.path.isdir(out_dir):
        print(f"\033[91m[CLI] Error: Directory '{in_dir}' or '{out_dir}' not found.\033[0m")
        print("\033[91m[CLI] Are you in the task directory (e.g. abc443/A) generated by 'atm gen'?\033[0m")
        return False
        
    in_files = sorted(glob.glob(os.path.join(in_dir, "*.txt")))
    
    if not in_files:
        print(f"\033[93m[CLI] No test cases found in '{in_dir}'.\033[0m")
        return True
    ...
    for in_file in in_files:
        ...
        if not os.path.isfile(out_file):
            print(f"\033[93m[CLI] Warning: Missing expected output file '{out_file}' for input '{basename}'. Skipping.\033[0m")
            continue
